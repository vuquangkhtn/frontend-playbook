...
const Demo = () => {
  const [reduceType, setReduceType] = useState<'sum' | 'product' | 'concat' | 'count'>('sum');
  const arr = [1, 2, 3, 4, 5];
  ...
  const result = arr.myReduce(getReduceFn(reduceType), getInitialValue(reduceType));
  ...
  return (
    <div style={{ padding: '20px', fontFamily: 'monospace' }}>
      ...
      <section style={{ marginBottom: '30px' }}>
        ...
        <p>
          The <code>reduce()</code> method executes a reducer function on each element 
          of the array, resulting in a single value. It accumulates values from left to 
          right and returns the final accumulated result.
        </p>
      </section>
          ...
          <div style={{ display: 'flex', gap: '10px', marginBottom: '10px', flexWrap: 'wrap' }}>
            <label style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
              <input
                ...
                onChange={(e) => setReduceType(e.target.value as 'sum' | 'product' | 'concat' | 'count')}
              />
              Sum (1+2+3+...)
            </label>
            <label style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
      ...
      <section style={{ marginBottom: '30px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
          ...
          <Example 
            ...
            result={[5, 2, 8, 1].myReduce((a: number, b: number) => Math.min(a, b))}
          />
          <Example 
            title="Max Value" 
            code="[5,2,8,1].myReduce((a,b)=>Math.max(a,b))" 
            result={[5, 2, 8, 1].myReduce((a: number, b: number) => Math.max(a, b))}
          />
        </div>
      </section>
      ...
      <section style={{ marginBottom: '30px' }}>
        ...
        <div style={{ padding: '10px', backgroundColor: '#f5f5f5', borderRadius: '4px', fontSize: '12px' }}>
          ...
          <ul style={{ marginLeft: '20px', lineHeight: '1.6' }}>
            <li><strong>Initial:</strong> accumulator = 0</li>
            <li><strong>Step 1:</strong> 0 + 1 = 1</li>
            <li><strong>Step 2:</strong> 1 + 2 = 3</li>
            <li><strong>Step 3:</strong> 3 + 3 = 6</li>
            <li><strong>Result:</strong> 6</li>
          </ul>
        </div>
      </section>
      ...
      <section>
        ...
        <ul>
          ...
          <li>✅ Optional initialValue (defaults to first element if omitted)</li>
          <li>✅ Returns a single accumulated value (any type)</li>
          <li>✅ Skips empty slots in sparse arrays</li>
          <li>✅ Throws error if array is empty and no initialValue provided</li>
        </ul>
      </section>
    </div>
  );
