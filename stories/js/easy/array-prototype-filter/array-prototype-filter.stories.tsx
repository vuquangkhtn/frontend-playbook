...
const Demo = () => {
  const [filterType, setFilterType] = useState<'even' | 'odd' | 'custom'>('even');
    ...
    <div style={{ padding: '20px', fontFamily: 'monospace' }}>
      ...
      <section style={{ marginBottom: '30px' }}>
        ...
        <p>
          ...
          that pass the test implemented by the provided callback function. It does 
          ...
          <div style={{ display: 'flex', gap: '10px', marginBottom: '10px' }}>
            <label style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
              <input
                ...
                onChange={(e) => setFilterType(e.target.value as 'even' | 'odd' | 'custom')}
              />
              Even Numbers
            </label>
            <label style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
      ...
      <section style={{ marginBottom: '30px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '15px' }}>
          <Example title="Filter Even" code="[1,2,3,4].myFilter(n => n % 2 === 0)" result={[1, 2, 3, 4].myFilter((n: number) => n % 2 === 0)} />
          <Example title="Filter Odd" code="[1,2,3,4].myFilter(n => n % 2 === 1)" result={[1, 2, 3, 4].myFilter((n: number) => n % 2 === 1)} />
          <Example title="Filter by Index" code="[10,20,30].myFilter((,i) => i % 2 === 0)" result={[10, 20, 30].myFilter((_, i) => i % 2 === 0)} />
          <Example title="Filter Greater Than" code="[1,5,3,8,2].myFilter(n => n > 3)" result={[1, 5, 3, 8, 2].myFilter((n: number) => n > 3)} />
          <Example title="Empty Result" code="[1,3,5].myFilter(n => n % 2 === 0)" result={[1, 3, 5].myFilter((n: number) => n % 2 === 0)} />
          <Example title="All Match" code="[2,4,6].myFilter(n => n % 2 === 0)" result={[2, 4, 6].myFilter((n: number) => n % 2 === 0)} />
        </div>
      </section>
