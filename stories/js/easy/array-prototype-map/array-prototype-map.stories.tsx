...
const Demo = () => {
  const [mapType, setMapType] = useState<"square" | "double" | "custom">(
    "square"
  );
  ...
  return (
    <div style={{ padding: "20px", fontFamily: "monospace" }}>
      ...
      <section style={{ marginBottom: "30px" }}>
        ...
        <p>
          The <code>map()</code> method creates a new array populated with the
          results of calling a callback function on every element in the array.
          It transforms each element and returns a new array without modifying
          the original.
        </p>
      </section>
          ...
          <div style={{ display: "flex", gap: "10px", marginBottom: "10px" }}>
            ...
            >
              <input
                ...
                onChange={(e) =>
                  setMapType(e.target.value as "square" | "double" | "custom")
                }
              />
              Square (n²)
            </label>
      ...
      <section style={{ marginBottom: "30px" }}>
        ...
        >
          <Example
            title="Square"
            code="[1,2,3].myMap(n => n * n)"
            result={[1, 2, 3].myMap((n: number) => n * n)}
          />
          <Example
            title="Double"
            code="[1,2,3].myMap(n => n * 2)"
            result={[1, 2, 3].myMap((n: number) => n * 2)}
          />
          <Example
            title="Multiply by Index"
            code="[10,20,30].myMap((n,i) => n * i)"
            result={[10, 20, 30].myMap((n: number, i) => n * i)}
          />
          <Example
            title="Stringify"
            code="[1,2,3].myMap(n => 'num' + n)"
            result={[1, 2, 3].myMap((n: number) => "num" + n)}
          />
          <Example
            title="Square Root"
            code="[4,9,16].myMap(Math.sqrt)"
            result={[4, 9, 16].myMap((n: number) => Math.sqrt(n))}
          />
          <Example
            title="Negate"
            code="[1,-2,3].myMap(n => -n)"
            result={[1, -2, 3].myMap((n: number) => -n)}
          />
        </div>
      </section>
      ...
      <section>
        ...
        <ul>
          ...
          <li>
            ✅ Can transform to different types (numbers to strings, etc.)
          </li>
        </ul>
      </section>
    </div>
...
const meta = {
  title: "JS/easy/Array.prototype.map",
  tags: ["!autodocs"],
  component: Demo,
