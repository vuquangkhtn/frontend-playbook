Solution
The general idea is to slice the input array into chunks of size elements and processing one chunk at a time. The solutions presented here use Promise.all but if it's not allowed, you can use the self-implemented versions presented in the Promise.all question.
...
Process items in the current chunk concurrently via Promise.all.
Recursively call mapAsyncLimit on the remaining items.
Base case is when there are no more items left to be processed and an empty array is returned.
Finally, merge the results from Step 1 and the mapped results from the remaining items in Step 2 and return it.
