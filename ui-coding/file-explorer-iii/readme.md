Note: This is a follow up to the File Explorer question, you should complete that question first before attempting this question.

In File Explorer, we built a functional file explorer component that displays an array of file objects in a hierarchical tree format using a nested DOM structure.
...
This flat approach is used by VS Code because of the need to handle large amount of files. In fact, VS Code uses position: absolute; left: Xpx; top: Ypx; for positioning and it only renders the visible file items.

Requirements
Modify the nested structure of the file explorer to a flat structure while retaining the same visual appearance.

Notes
The skeleton code uses the solution of File Explorer, but you are free to use your own solution as a starting point.